...
class HttpSignatureSpecTest  extends FreeSpec {
  ...
  //https://stackoverflow.com/questions/4032985/how-do-we-convert-a-string-from-pem-to-der-format
    ...
    //  val spec = new RSAPublicKeySpec(pk.getModulus,pk.getPublicExponent)
  ...
  // - https://www.w3.org/TR/xmldsig-core/
    ...
    val decode = (base64Str: String) => BigIntegers.fromUnsignedByteArray(Base64.getDecoder.decode(base64Str))
...
//        |""".stripMargin.trim
    ...
    "signature tests" - {
      ...
      "verify public/private key pairs from spec" - {
      ...
      """C.1 default test.
        ...
        "read date for 5 Jan 2012, not in spec, but matches signature in spec" in {
          val signatureInSpecFor2014 = """
            |ATp0r26dbMIxOopqw0OfABDT7CKMIoENumuruOtarj8n/97Q3htH
            |FYpH8yOSQk3Z5zh8UxUym6FYTb5+A0Nz3NRsXJibnYi7brE/4tx5But9kkFGzG+
            |xpUmimN4c3TMN7OFH//+r8hBf7BT9/GmHDUVZT2JzWGLZES2xDOUuMtA=""".trim.stripMargin.lines.mkString("")
 
          val fictionalDateStr = "date: Thu, 05 Jan 2012 21:31:40 GMT"
          ...
          calculatedSignature should be(signatureInSpecFor2014)
        }
  
        "fictional date for 5 jan 2014, in spec, matches different signature" in {
          val weExpectSig_butNotSpec = """
          |jKyvPcxB4JbmYY4mByyBY7cZfNl4OW9HpFQlG7N4YcJPteKTu4MWCLyk
          |+gIr0wDgqtLWf9NLpMAMimdfsH7FSWGfbMFSrsVTHNTk0rK3usrfFnti
          |1dxsM4jl0kYJCKTGI/UWkqiaxwNiKqGcdlEDrTcUhhsFsOIo8VhddmZTZ8w=
          """.trim.stripMargin.lines.mkString
          val fictionalDateStr = "date: Thu, 05 Jan 2014 21:31:40 GMT"
          val asi = new SigInfo(List(), HttpSignature.`rsa-sha256`, "Test", fictionalDateStr)
          val calculatedSignature = asi.sign(rsaPrivateKey).get.encodedSig
          calculatedSignature should be(weExpectSig_butNotSpec)
        }
        
        
        "actual date for 5 jan 2014" in {
          val signatureNotInSpecButExpected = """
          |SjWJWbWN7i0wzBvtPl8rbASWz5xQW6mcJmn+ibttBqtifLN7Sazz6m79cNf
          |wwb8DMJ5cou1s7uEGKKCs+FLEEaDV5lp7q25WqS+lavg7T8hc0GppauB6hbgE
          |KTwblDHYGEtbGmtdHgVCk9SuS13F0hZ8FD0k/5OxEPXe5WozsbM=
          """.trim.stripMargin.lines.mkString
          val tobesigned = "date: Sun, 05 Jan 2014 21:31:40 GMT"
